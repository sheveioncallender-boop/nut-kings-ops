...
const OP_LABELS = {
  ...
  finished_to_truck: 'Issue Finished Goods to Truck',
...
async function checkServer() {
  ...
  try {
    await fetchJSON('/nutkings/api/ping', {}, 7000);
    ...
    for (const result of data.results || []) {
      ...
      if (result.status === 'processed') {
        ...
        item.sync_error = result.status === 'needs_action'
          ? 'Odoo requires an additional confirmation. Open the transfer in Backend while online.'
...
function renderAll() {
  ...
  $('developer-link').hidden = !(snapshot.permissions?.manager);
...
function applyPermissions() {
  ...
  $('open-native-count').hidden = !(state.serverOnline && permissions.manager);
}
...
function renderDashboard() {
  ...
  const localOperations = state.queue.filter((item) => ['stock_operation', 'create_transfer'].includes(item.kind)).slice(-5).reverse().map((item) => ({
...
function localOperationRows() {
  return state.queue.filter((item) => ['stock_operation', 'create_transfer', 'operation_event', 'picking_action'].includes(item.kind)).map((item) => {
...
function renderReview(operation, editableActions = false) {
  ...
  if (operation.local) {
    ...
    if (operation.web_url) buttons.push(`<a class="nk-button light" href="${html(operation.web_url)}">Backend</a>`);
  }
...
async function saveTrip() {
  ...
  if (!truckId || !driverId || !routeName || ((state.snapshot.service_areas || []).length && !serviceAreaId)) {
    toast('Truck, driver, service area, and route are required.', 'error');
    return;
  }
  const item = {
...
function cleanOpenProduct(productId) {
  ...
  if (!product) return;
  if (state.serverOnline && product.backend_url) {
  ...
  $('product-detail-content').innerHTML = `
    <div class="nk-warning-note">Offline inventory snapshot · Last synchronized ${state.snapshot.saved_at ? formatDateTime(state.snapshot.saved_at) : 'never'}.</div>
    ...
    <div class="nk-card" style="margin-top:15px"><h3>Reserved / Used By</h3>
      <div class="nk-table-wrap"><table class="nk-table"><thead><tr><th>Reference</th><th>Customer / Company</th><th>Truck / Trip</th><th class="number">Demand</th><th class="number">Reserved</th><th>Status</th></tr></thead><tbody>
  ...
  $('open-odoo-product').hidden = !state.serverOnline || !product.backend_url;
...
function bindCleanFeatureEvents() {
  ...
  $('open-odoo-product')?.addEventListener('click', () => {
    ...
    if (product?.backend_url) window.open(product.backend_url, '_blank', 'noopener');
  });
...
async function init() {
  ...
  if ('serviceWorker' in navigator) {
    try {
      const registration = await navigator.serviceWorker.register('/nutkings/sw.js?v=1.3.2', { scope: '/nutkings/' });
