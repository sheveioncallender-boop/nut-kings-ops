...
class NutKingsApi(http.Controller):
    ...
    def bootstrap(self, **kwargs):
        ...
        trucks = request.env['nutkings.truck'].sudo().search([
            ('company_id', '=', request.env.company.id), ('active', '=', True),
        ], order='name')
        trips = request.env['nutkings.trip'].sudo().search([
            ('company_id', '=', request.env.company.id),
        ], order='planned_departure desc, id desc', limit=250)
        pickings = request.env['stock.picking'].sudo().search([
            ('nk_is_operation', '=', True), ('company_id', '=', request.env.company.id),
        ], order='scheduled_date desc, id desc', limit=300)
        ...
        contact_domain = [('active', '=', True), ('type', 'in', (False, 'contact'))]
        customers = Partner.search(contact_domain, order='nk_is_customer desc, name')
        suppliers = Partner.search(contact_domain, order='nk_is_supplier desc, name')
        ...
        return request.make_json_response({
            ...
            'permissions': {'raw': True, 'finished': True, 'distribution': True, 'supervisor': True, 'manager': True, 'system': True},
            'capabilities': ['raw_receipt', 'raw_issue', 'finished_receipt', 'finished_to_truck', 'customer_delivery', 'truck_return'],
            'native_actions': {
    ...
    def _create_transfer(cls, item, sync_event):
        ...
        for line in lines:
            ...
            if not product or product.nk_inventory_type != config['product_type']:
        ...
        picking = request.env['stock.picking'].sudo().create({
            ...
            'nk_reason_id': cls._safe_int(item.get('reason_id')) or False,
    ...
    def _apply_inventory_count(cls, item, sync_event):
        ...
        lines = item.get('lines') or []
        if not lines:
            raise ValueError('Add at least one counted line.')
        ...
        for line in lines:
            product = request.env['product.product'].sudo().browse(cls._safe_int(line.get('product_id'))).exists()
            if not product or product.nk_inventory_type != inventory_type:
            ...
            if not quant:
                ...
                if product.tracking != 'none' and not lot_id:
                    ...
                    lot = request.env['stock.lot'].sudo().search([('name', '=', lot_reference), ('product_id', '=', product.id)], limit=1)
                    if not lot:
                        lot = request.env['stock.lot'].sudo().create({'name': lot_reference, 'product_id': product.id, 'company_id': request.env.company.id})
                    lot_id = lot.id
                quant = Quant.create({'product_id': product.id, 'location_id': location.id, 'lot_id': lot_id or False, 'inventory_quantity': counted})
            else:
    ...
    def _create_trip(cls, item, sync_event):
        ...
        trip = request.env['nutkings.trip'].sudo().create({
            'truck_id': truck.id,
            'driver_id': cls._safe_int(item.get('driver_id')) or False,
            'team_ids': [Command.set([int(value) for value in item.get('team_ids') or []])],
            'customer_ids': [Command.set([int(value) for value in item.get('customer_ids') or []])],
            'route_name': str(item.get('route') or '').strip()[:256] or 'Unspecified Route',
    ...
    def sync(self, **kwargs):
        ...
        for item in transactions:
            ...
            existing = Event.search([('external_uid', '=', external_uid)], limit=1)
